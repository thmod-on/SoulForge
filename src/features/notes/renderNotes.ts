...
export function renderNotes(character: Character, dependencies: NotesRenderDependencies): string {
  ...
  return `<main class="content"><div class="screen-title"><div><h1>Anotacoes</h1><p>Registre lembretes, pistas e detalhes importantes da campanha.</p></div><button class="sf-action sf-action--primary primary-action screen-title-action" type="button" data-action="open-note-modal">Nova anotacao</button></div>${notes.length ? `<div class="notes-grid">${notes.map((note) => renderNoteCard(note, dependencies)).join("")}</div>` : dependencies.renderEmptyInline("Nenhuma anotacao registrada.")}</main>`;
}
...
export function renderNoteModal(dependencies: NotesRenderDependencies): string {
  ...
  return `<div class="modal-backdrop" data-modal-backdrop><section class="note-modal sf-scroll-region" role="dialog" aria-modal="true" aria-labelledby="note-modal-title"><div class="container-modal-heading"><h2 id="note-modal-title">${note ? "Editar anotacao" : "Nova anotacao"}</h2><button class="modal-close modal-close-inline" data-modal-close aria-label="Fechar anotacao">x</button></div><p class="form-error" data-note-error hidden></p><label><span>Titulo</span><input data-note-title type="text" value="${escapeHtml(note?.title ?? "")}" placeholder="Ex.: Nome do contato misterioso" /></label><div class="note-category-field"><span>Categoria</span><div class="note-category-options">${(Object.keys(noteCategoryLabels) as CharacterNoteCategory[]).map((category) => `<button class="sf-tab ${selectedCategory === category ? "is-active" : ""}" type="button" data-note-category-option="${category}">${noteCategoryLabels[category]}</button>`).join("")}</div><input data-note-category type="hidden" value="${selectedCategory}" /></div><label><span>Conteudo</span><textarea data-note-content rows="8" placeholder="Anote pistas, promessas, NPCs ou ideias da sessao...">${escapeHtml(note?.content ?? "")}</textarea></label><button class="sf-action sf-action--primary primary-action" type="button" data-action="save-note">Salvar anotacao</button></section></div>`;
}
