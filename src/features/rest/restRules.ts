...
export type RestMoveId = "tend-wounds" | "clear-stress" | "repair-armor" | "prepare" | "work-project";
export type RestMoveChoice = { id: RestMoveId; roll?: number };

export const restMoveLabels: Record<RestMoveId, string> = { "tend-wounds": "Tratar ferimentos", "clear-stress": "Limpar estresse", "repair-armor": "Reparar armadura", prepare: "Preparar", "work-project": "Trabalhar em projeto" };

export function getRestMoves(kind: RestKind): RestMoveId[] { return kind === "short" ? ["tend-wounds", "clear-stress", "repair-armor", "prepare"] : ["tend-wounds", "clear-stress", "repair-armor", "prepare", "work-project"]; }
export function restMoveDescription(kind: RestKind, move: RestMoveId): string { if (move === "prepare") return "Ganhe 1 Esperança."; if (move === "work-project") return "Ação narrativa. Projetos terão acompanhamento em uma etapa futura."; const name = move === "tend-wounds" ? "PV" : move === "clear-stress" ? "Estresse" : "Armadura"; return kind === "short" ? `Limpe 1d4 + Tier de ${name}.` : `Limpe todo ${name}.`; }
...
export function applyRestMoves(character: Character, kind: RestKind, moves: RestMoveChoice[]): Character {
  ...
  for (const move of moves) {
    ...
    const amount = kind === "long" ? resource.max : move.id === "prepare" ? 1 : Math.max(0, Math.min(4, move.roll ?? 0)) + tier;
    resources = resources.map((entry) => entry.id === resource.id ? { ...entry, value: Math.min(entry.max, entry.value + amount) } : entry);
  }
