...
import { getRestMoves, requiresRestRoll, restMoveDescription, restMoveLabels, type DefinitionRestMoveChoice, type RestKind, type RestMoveChoice } from "./restRules";
...
export function renderRestModal(character: Character | undefined, kind: RestKind | undefined, choices: RestMoveChoice[], error: string | undefined, deps: RestRenderDependencies): string {
  ...
  const options = getRestMoves(kind).map((move) => {
    ...
    return `<button type="button" class="rest-move-option ${count ? "is-selected" : ""}" data-action="choose-rest-move" data-rest-move="${move}" ${selected >= 2 && !count ? "disabled" : ""}><strong>${restMoveLabels[move]}</strong><span>${restMoveDescription(kind, move)}</span>${count ? `<small>${count} ${count === 1 ? "escolha" : "escolhas"}</small>` : ""}</button>`;
  }).join("");
  const contextual = renderDefinitionActionOption(character, choices, selected, deps);
  const selectedMoves = choices.map((choice, index) => renderSelectedMove(kind, choice, index, deps)).join("");
  return `<div class="modal-backdrop" data-modal-backdrop><section class="rest-modal sf-scroll-region" role="dialog" aria-modal="true" aria-labelledby="rest-modal-title"><button class="modal-close" type="button" data-modal-close aria-label="Fechar descanso">×</button><div class="rest-modal-heading"><h2 id="rest-modal-title">Descanso</h2></div><div class="rest-kind-switch" role="tablist" aria-label="Tipo de descanso"><button class="sf-tab ${kind === "short" ? "is-active" : ""}" type="button" data-action="set-rest-kind" data-rest-kind="short" role="tab" aria-selected="${kind === "short"}">Breve</button><button class="sf-tab ${kind === "long" ? "is-active" : ""}" type="button" data-action="set-rest-kind" data-rest-kind="long" role="tab" aria-selected="${kind === "long"}">Longo</button></div><p class="rest-guidance">Escolha dois movimentos.</p><div class="rest-move-options">${options}${contextual}</div><section class="rest-selections"><div class="section-heading"><h3>Movimentos escolhidos</h3><span>${selected}/2</span></div>${selectedMoves || "<p>Escolha até dois movimentos de descanso.</p>"}</section>${error ? `<p class="form-error">${deps.escapeHtml(error)}</p>` : ""}<div class="modal-actions"><button class="sf-action sf-action--secondary secondary-action" type="button" data-modal-close>Cancelar</button><button class="sf-action sf-action--primary primary-action" type="button" data-action="confirm-rest" ${selected !== 2 ? "disabled" : ""}>Concluir descanso</button></div></section></div>`;
}

function renderDefinitionActionOption(character: Character, choices: RestMoveChoice[], selected: number, deps: RestRenderDependencies): string {
  ...
  const unavailable = !active || chosen || selected >= 2;
  const status = chosen ? "Movimento escolhido" : !active ? unavailableMessage : selected >= 2 ? "Remova um movimento para escolher este." : "Ocupa um movimento de descanso.";
