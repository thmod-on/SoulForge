...
function renderRemoveInstalledPackModal(deps: Pick<PackManagementDependencies, "state" | "escapeHtml">): string {
  ...
  if (!pack) return "";
  return `<div class="modal-backdrop" data-modal-backdrop><section class="container-modal danger-modal" role="dialog" aria-modal="true" aria-labelledby="remove-pack-title"><button class="modal-close" type="button" data-action="cancel-remove-installed-pack" aria-label="Cancelar remoção">×</button><span class="resource-modal-label">Dados locais</span><h2 id="remove-pack-title">Remover Pack?</h2><p><strong>${escapeHtml(pack.name)}</strong> e todas as suas Definitions serão removidos deste dispositivo.</p><div class="danger-summary"><strong>!</strong><span>Personagens que usem esse conteúdo poderão ficar com referências indisponíveis.</span></div><div class="confirmation-actions"><button class="sf-action sf-action--secondary secondary-action" type="button" data-action="cancel-remove-installed-pack">Cancelar</button><button class="sf-action sf-action--danger danger-action" type="button" data-action="confirm-remove-installed-pack">Remover Pack</button></div></section></div>`;
}
...
function renderRemoveAllInstalledPacksModal(deps: Pick<PackManagementDependencies, "state" | "escapeHtml">): string {
  ...
  return `<div class="modal-backdrop" data-modal-backdrop><section class="container-modal danger-modal remove-all-packs-modal" role="dialog" aria-modal="true" aria-labelledby="remove-all-packs-title"><button class="modal-close" type="button" data-action="cancel-remove-all-installed-packs" aria-label="Cancelar remoção">×</button><span class="resource-modal-label">Dados locais</span><h2 id="remove-all-packs-title">Remover todos os Packs?</h2><p>Os <strong>${count} ${count === 1 ? "Pack importado" : "Packs importados"}</strong> e suas Definitions serão removidos deste dispositivo em uma única operação.</p><div class="remove-all-packs-list sf-scroll-region">${state.installedPacks.map((pack) => `<span><strong>${escapeHtml(pack.name)}</strong><small>v${escapeHtml(pack.version)}</small></span>`).join("")}</div><div class="danger-summary"><strong>!</strong><span>Fichas que usam estes Packs podem mostrar referências indisponíveis até que eles sejam reimportados.</span></div><p>Personagens, conteúdo criado manualmente, configurações e complementos locais serão preservados.</p>${state.removeAllInstalledPacksError ? `<p class="form-error" role="alert">${escapeHtml(state.removeAllInstalledPacksError)}</p>` : ""}<div class="confirmation-actions"><button class="sf-action sf-action--secondary secondary-action" type="button" data-action="cancel-remove-all-installed-packs">Cancelar</button><button class="sf-action sf-action--danger danger-action" type="button" data-action="confirm-remove-all-installed-packs">Remover todos os Packs</button></div></section></div>`;
}
