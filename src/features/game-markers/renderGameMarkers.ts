...
function renderMarker(marker: ActiveGameMarker, escapeHtml: (value: string) => string): string {
  ...
  return `<article class="game-marker-preview game-marker-dice"><small>${escapeHtml(marker.sourceLabel)}</small><h3>${escapeHtml(marker.definition.label)}</h3><p class="game-marker-dice-help">Defina o resultado de cada dado e marque individualmente qual foi usado.</p><div class="game-marker-dice-list">${diceState.results.map((die, index) => `<div class="game-marker-die ${die.used ? "is-used" : ""}"><span>${diceState.die} ${index + 1}</span><div><small class="game-marker-die-result-label">Resultado: ${die.value || "—"}</small><div class="game-marker-die-results" aria-label="Escolher resultado do dado ${index + 1}">${[1, 2, 3, 4].map((value) => `<button type="button" class="${die.value === value ? "is-selected" : ""}" data-game-marker-die-result="${escapeHtml(marker.key)}" data-game-marker-die-id="${escapeHtml(die.id)}" data-game-marker-die-value="${value}" aria-pressed="${die.value === value}">${value}</button>`).join("")}</div></div><button class="game-marker-die-use" type="button" data-game-marker-die-use="${escapeHtml(marker.key)}" data-game-marker-die-id="${escapeHtml(die.id)}" ${die.value === 0 ? "disabled" : ""}>${die.used ? "Disponibilizar" : "Marcar usado"}</button></div>`).join("")}</div><span>${diceState.results.filter((die) => die.used).length} usado(s)${marker.definition.reset ? ` · Reinicia: ${resetLabel(marker.definition.reset)}` : ""}</span></article>`;
}
