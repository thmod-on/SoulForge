...
export function renderProgression(character: Character, dependencies: ProgressionRenderDependencies): string {
  ...
  const xp = `<div class="progression-xp-status"><span>Experiência</span><strong>${character.identity.xp} / ${character.identity.nextLevelXp} XP</strong><progress value="${Math.max(0, character.identity.xp)}" max="${Math.max(1, character.identity.nextLevelXp)}" aria-label="Experiência até o próximo nível"></progress></div>`;
  if (state.progressionCompletionLevel) {
    const tier = getTierForLevel(character.identity.level);
    return `<main class="content progression-content"><div class="screen-title progression-title"><div><h1>Progressão</h1><p>Nível ${character.identity.level} · Tier ${tier}</p></div><button class="sf-action sf-action--secondary sf-action--compact progression-history-button" type="button" data-action="open-progression-history"><span aria-hidden="true">↶</span> Histórico</button></div>${xp}<section class="progression-completion" role="status" aria-live="polite"><img src="assets/brand/soulforge-symbol.png" alt="" aria-hidden="true" /><div><span>Progressão concluída</span><h2>Nível ${state.progressionCompletionLevel} alcançado</h2><p>Sua ficha foi atualizada.</p></div></section></main>`;
  }
