...
import { cardReactivationLabels, getUnavailableCard, hasCardsAwaitingReactivation } from "./cardAvailability";
...
export type PlayerOverviewDependencies = {
  ...
  renderTransformation: (character: Character) => string;
...
export function renderOverview(character: Character, dependencies: PlayerOverviewDependencies): string {
  ...
  const hasSessionReset = gameMarkers.some((marker) => marker.definition.reset === "session") || hasCardsAwaitingReactivation(character, "session");
  return `<main class="content">${renderActiveFeatureEffects(character, activeFeatureEffects, activeSheetModifierEffects, dependencies.escapeHtml)}${dependencies.renderResources(character)}${renderGameMarkers(gameMarkers, dependencies.escapeHtml)}${dependencies.renderTransformation(character)}<section class="band active-card-band"><div class="section-heading"><h2>Cartas ativas</h2><span>${activeCards.length} / 5 ativas</span></div><div class="active-card-rail"><button class="active-card-carousel-control is-previous" type="button" data-action="scroll-active-card-carousel" data-active-card-direction="-1" aria-label="Ver cartas anteriores" title="Cartas anteriores" hidden>‹</button><div class="card-row active-card-track" data-active-card-carousel tabindex="0" aria-label="Cartas ativas no Loadout">${activeCards.map((card) => renderCardTile(card, dependencies, character)).join("")}</div><button class="active-card-carousel-control is-next" type="button" data-action="scroll-active-card-carousel" data-active-card-direction="1" aria-label="Ver próximas cartas" title="Próximas cartas" hidden>›</button></div><button class="sf-action sf-action--secondary deck-drawer-button" data-action="open-stored-cards">Ver Vault (${inactiveCards})</button></section>${renderSubclassTrack(character, dependencies)}${renderMulticlassTrack(character, dependencies)}<section class="quick-actions"><button class="sf-action sf-action--secondary" data-action="open-rest" aria-label="Abrir descanso" title="Descanso"><span aria-hidden="true">🛏</span> Descanso</button>${hasSessionReset ? '<button class="sf-action sf-action--secondary" data-action="reset-game-markers-session"><span>NEW</span> Nova sessao</button>' : ""}<button class="sf-action sf-action--secondary" data-page="skills"><span>XP</span> Registrar experiencia</button></section></main>${renderSubclassFeatureModal(character, dependencies)}`;
}
...
export function renderCardTile(card: CardDefinition, dependencies: Pick<PlayerOverviewDependencies, "escapeHtml" | "getDomainInfo">, character?: Character): string {
  const { escapeHtml } = dependencies;
  const domain = dependencies.getDomainInfo(card.domainId);
  const domainColor = domain?.color ?? "#8e4fc4";
  const unavailable = character ? getUnavailableCard(character, card.id) : undefined;
  if (unavailable) return `<button class="ability-card is-unavailable" data-card-modal-id="${card.id}" style="--card-domain-color: ${escapeHtml(domainColor)}" aria-label="${escapeHtml(card.name)}, indisponível ${cardReactivationLabels[unavailable.reactivation]}"><span class="unavailable-card-back" aria-hidden="true"><img class="unavailable-card-watermark" src="assets/brand/soulforge-symbol.png" alt="" /><strong class="unavailable-card-name">${escapeHtml(card.name)}</strong><span class="unavailable-card-status"><strong>Carta indisponível</strong><small>Reativa ${cardReactivationLabels[unavailable.reactivation]}</small></span></span></button>`;
...
function renderStoredCardTile(card: CardDefinition, dependencies: PlayerOverviewDependencies): string {
  return `<article class="stored-card">${renderCardTile(card, dependencies)}<button class="sf-action sf-action--secondary stored-card-action" type="button" data-action="activate-stored-card" data-card-id="${card.id}" aria-label="Mover ${dependencies.escapeHtml(card.name)} para o Loadout"><span aria-hidden="true">↥</span><span>Mover para o Loadout</span></button></article>`;
}
