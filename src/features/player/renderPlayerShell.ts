...
export type PlayerShellDependencies = {
  ...
  getCommunityName: (character: Character) => string | undefined;
};

export function renderSidebar(character: Character, dependencies: PlayerShellDependencies): string {
  const { appVersion, escapeHtml, attributeTitle, getEffectiveDefense, getSpellcastAttributeId, getCommunityName } = dependencies;
  ...
  const community = getCommunityName(character); return `<aside class="sidebar"><div class="portrait"><button class="portrait-art ${portrait ? "has-image is-expandable" : ""}" type="button"${portraitAction} aria-label="${portrait ? `Ampliar retrato de ${escapeHtml(character.identity.name)}` : "Retrato indisponivel"}" ${portrait ? "" : "disabled"}>${portrait}</button><div><strong>${escapeHtml(character.identity.name)}</strong><span>${escapeHtml(character.identity.ancestry)} - ${escapeHtml(character.identity.className)}</span>${community ? `<small>${escapeHtml(community)}${character.identity.community ? ` · ${escapeHtml(character.identity.community)}` : ""}</small>` : ""}</div><button class="portrait-change" type="button" data-action="open-character-portrait">Alterar foto</button></div><section class="sidebar-section"><div class="sidebar-section-title">Atributos</div><div class="sidebar-attribute-grid">${character.attributes.map((attribute) => { const isSpellcast = attribute.id === spellcastAttributeId; return `<div class="attribute-badge ${attribute.upgraded ? "is-upgraded" : ""} ${isSpellcast ? "is-spellcast" : ""}"><span title="${attribute.label}">${attributeTitle(attribute.label)}</span>${isSpellcast ? '<i class="attribute-spellcast-icon" role="img" aria-label="Atributo de Conjuração" title="Atributo de Conjuração">🪄</i>' : ""}<strong>${attribute.value}</strong></div>`; }).join("")}</div></section><section class="sidebar-section"><div class="sidebar-section-title">Combate</div><div class="sidebar-defense-grid"><div class="defense-badge defense-evasion"><strong>${defense.evasion}</strong><span>Evasao</span></div><div class="defense-badge defense-armor"><strong>${defense.armor}</strong><span>Armadura</span></div><div class="defense-badge defense-minor"><strong>${defense.minor}</strong><span>Limiar menor</span></div><div class="defense-badge defense-major"><strong>${defense.major}</strong><span>Limiar maior</span></div></div><div class="proficiency-marker" title="Proficiencia"><span aria-hidden="true">⚄</span><small>Proficiencia</small><strong>${character.proficiency}</strong></div></section><footer class="sidebar-footer" title="A PWA instala o app e guarda a interface para uso offline."><button class="sidebar-character-button" type="button" data-action="open-character-select" title="Trocar personagem">Personagens</button><span class="sidebar-offline-status"><i aria-hidden="true"></i>Pronto offline</span><span>v${appVersion}</span></footer></aside>`;
}
