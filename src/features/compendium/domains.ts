...
export function renderCompendiumDomainsManager(dependencies: DomainFeatureDependencies): string {
  ...
  return `<main class="content compendium-content"><div class="screen-title"><div><h1>Dominios</h1><p>Organize as vertentes que classificam as cartas do seu Compendium.</p></div><button class="secondary-action screen-title-action" type="button" data-action="back-compendium-index">Voltar ao indice</button></div><section class="compendium-management-panel"><div class="compendium-management-toolbar"><div class="management-copy"><span>BASE DO COMPENDIUM</span><strong>${sortedDomains.length} ${sortedDomains.length === 1 ? "dominio" : "dominios"}</strong></div><button class="primary-action" type="button" data-action="new-compendium-domain">Novo dominio</button></div><div class="compendium-domain-results">${sortedDomains.map((domain) => renderCompendiumDomainResult(domain, dependencies)).join("")}</div></section></main>`;
}
