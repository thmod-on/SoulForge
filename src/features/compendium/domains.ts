...
export function renderDomainModal(dependencies: DomainFeatureDependencies): string {
  ...
  return `<div class="modal-backdrop" data-modal-backdrop><section class="form-modal compendium-form-modal" role="dialog" aria-modal="true" aria-labelledby="domain-modal-title"><button class="modal-close" data-modal-close aria-label="Fechar dominio">x</button><header class="compendium-form-header"><span class="resource-modal-label">Compendium</span><h2 id="domain-modal-title">${existing ? "Editar dominio" : "Novo dominio"}</h2><p>O dominio sera oferecido como classificacao obrigatoria ao criar cartas.</p></header><div class="compendium-form-body"><label class="form-field"><span>Nome *</span><input data-domain-name value="${escapeHtml(existing?.name ?? "")}" placeholder="Ex.: Arcano" /></label><label class="form-field"><span>Descricao *</span><textarea data-domain-summary placeholder="Explique a proposta deste dominio.">${escapeHtml(existing?.summary ?? "")}</textarea></label><label class="form-field form-color-field"><span>Cor de identidade</span><input data-domain-color type="color" value="${escapeHtml(existing?.color ?? "#8e4fc4")}" /></label></div><footer class="compendium-form-footer"><p class="form-error" data-domain-error hidden></p><div class="modal-actions"><button class="sf-action sf-action--secondary secondary-action" type="button" data-modal-close aria-label="Cancelar" title="Cancelar">Cancelar</button><button class="sf-action sf-action--primary primary-action" type="button" data-action="save-compendium-domain" aria-label="Gravar dominio" title="Gravar dominio">Salvar</button></div></footer></section></div>`;
}
...
export function renderDeleteDomainModal(dependencies: DomainFeatureDependencies): string {
  ...
  return `<div class="modal-backdrop" data-modal-backdrop><section class="container-modal danger-modal" role="dialog" aria-modal="true" aria-labelledby="delete-domain-title"><button class="modal-close" type="button" data-action="cancel-delete-domain" aria-label="Cancelar exclusão">×</button><span class="resource-modal-label">Compendium</span><h2 id="delete-domain-title">Excluir domínio?</h2><p>O domínio <strong>${escapeHtml(domain.name)}</strong> ${linkedCards ? `possui ${linkedCards} ${linkedCards === 1 ? "carta vinculada" : "cartas vinculadas"}.` : "será removido deste dispositivo."}</p><div class="danger-summary"><strong>!</strong><span>${linkedCards ? "Transfira as cartas vinculadas antes de excluir o domínio." : "Esta ação não pode ser desfeita."}</span></div><div class="confirmation-actions"><button class="sf-action sf-action--secondary secondary-action" type="button" data-action="cancel-delete-domain">Cancelar</button>${linkedCards ? "" : '<button class="sf-action sf-action--danger danger-action" type="button" data-action="confirm-delete-domain">Excluir domínio</button>'}</div></section></div>`;
}
