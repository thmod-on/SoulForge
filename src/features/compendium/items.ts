...
export async function saveCompendiumItem(dependencies: ItemFeatureDependencies): Promise<void> {
  ...
  if (!name || !validCategory || !Number.isFinite(weight) || weight < 0 || !summary || duplicate || (tier !== undefined && (!Number.isInteger(tier) || tier < 1 || tier > 4)) || (itemValue !== undefined && (!Number.isFinite(itemValue) || itemValue < 0))) { if (error) { error.hidden = false; error.textContent = duplicate ? "Ja existe um item com este nome." : "Preencha nome, categoria, peso e descricao. Tier deve ficar entre 1 e 4."; } return; }
  ...
  await saveCustomDefinition({ id: existing?.id ?? `item.local.${crypto.randomUUID()}`, type: "item", packId: "local", name, summary, category, tier, weight, value: itemValue, traits: traits.length ? traits : undefined, image });
