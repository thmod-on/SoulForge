...
export function renderCompendiumAncestryFormModal(deps: AncestryFeatureDependencies): string {
  ...
  return `<div class="modal-backdrop" data-modal-backdrop><form class="modal ancestry-form-modal compendium-form-modal" onsubmit="return false;" aria-labelledby="ancestry-form-title"><button class="modal-close" type="button" data-modal-close aria-label="Fechar">×</button><header class="compendium-form-header"><div class="modal-title"><span>Compendium</span><h2 id="ancestry-form-title">${ancestry ? "Editar ancestralidade" : "Nova ancestralidade"}</h2><p>As duas features ficam ordenadas para suportar escolhas unicas e mistas.</p></div></header><div class="compendium-form-body"><label class="form-field"><span>Nome</span><input data-ancestry-name required value="${deps.escapeHtml(ancestry?.name ?? "")}" /></label><label class="form-field"><span>Descricao</span><textarea data-ancestry-summary required>${deps.escapeHtml(ancestry?.summary ?? "")}</textarea></label><label class="form-field"><span>Imagem (opcional)</span><input data-ancestry-image type="file" accept="image/*" />${ancestry?.image ? "<small>A imagem atual sera mantida se nenhum arquivo for escolhido.</small>" : ""}</label><div class="compendium-form-feature-pair">${featureFields("Top", top, "top", deps)}${featureFields("Bottom", bottom, "bottom", deps)}</div></div><footer class="compendium-form-footer"><p class="form-error" data-ancestry-error hidden></p><div class="modal-actions"><button class="sf-action sf-action--secondary secondary-action" type="button" data-modal-close>Cancelar</button><button class="sf-action sf-action--primary primary-action" type="button" data-action="save-compendium-ancestry">Salvar ancestralidade</button></div></footer></form></div>`;
}
...
export function renderDeleteCompendiumAncestryModal(deps: AncestryFeatureDependencies): string {
  ...
  return ancestry ? `<div class="modal-backdrop" data-modal-backdrop><section class="container-modal danger-modal" role="dialog" aria-modal="true" aria-labelledby="delete-compendium-ancestry-title"><button class="modal-close" type="button" data-action="cancel-delete-compendium-ancestry" aria-label="Cancelar exclusão">×</button><span class="resource-modal-label">Compendium</span><h2 id="delete-compendium-ancestry-title">Excluir ancestralidade?</h2><p>“${deps.escapeHtml(ancestry.name)}” e suas duas Features serão removidas deste dispositivo.</p><div class="danger-summary"><strong>!</strong><span>Personagens que a referenciam poderão perder acesso a esse conteúdo.</span></div><div class="confirmation-actions"><button class="sf-action sf-action--secondary secondary-action" type="button" data-action="cancel-delete-compendium-ancestry">Cancelar</button><button class="sf-action sf-action--danger danger-action" type="button" data-action="confirm-delete-compendium-ancestry">Excluir ancestralidade</button></div></section></div>` : "";
}
