...
export function renderCompendiumCardFormModal(dependencies: CardFeatureDependencies): string {
  ...
  if (card && card.packId !== "local") {
    return `<div class="modal-backdrop" data-modal-backdrop><section class="form-modal card-form-modal compendium-form-modal" role="dialog" aria-modal="true" aria-labelledby="card-form-modal-title"><button class="modal-close" data-modal-close aria-label="Fechar marcador">x</button><header class="compendium-form-header"><h2 id="card-form-modal-title">Complementar marcador</h2><p><strong>${escapeHtml(card.name)}</strong> pertence a um pack e permanece inalterada. Este complemento e salvo somente neste dispositivo.</p></header><div class="compendium-form-body">${renderGameMarkerFields("pack-card", card.gameMarkers?.[0], escapeHtml)}</div><footer class="compendium-form-footer"><p class="form-error" data-compendium-card-error hidden></p><div class="modal-actions"><button class="sf-action sf-action--secondary secondary-action" type="button" data-modal-close aria-label="Cancelar" title="Cancelar">Cancelar</button><button class="sf-action sf-action--primary primary-action" type="button" data-action="save-pack-card-marker" aria-label="Salvar marcador" title="Salvar marcador">Salvar</button></div></footer></section></div>`;
  }
  const domains = [...catalog.domains].sort((a, b) => a.name.localeCompare(b.name, "pt-BR"));
  return `<div class="modal-backdrop" data-modal-backdrop><section class="form-modal card-form-modal compendium-form-modal" role="dialog" aria-modal="true" aria-labelledby="card-form-modal-title"><button class="modal-close" data-modal-close aria-label="Fechar carta">x</button><header class="compendium-form-header"><h2 id="card-form-modal-title">${card ? "Editar carta" : "Nova carta"}</h2><p>A carta sera salva localmente e ficara disponivel para os fluxos compativeis deste dispositivo.</p></header><div class="compendium-form-body"><div class="form-grid"><label class="form-field"><span>Nome *</span><input data-compendium-card-name value="${escapeHtml(card?.name ?? "")}" placeholder="Ex.: Passo Sombrio" /></label><label class="form-field"><span>Dominio *</span><select data-compendium-card-domain>${domains.map((domain) => `<option value="${domain.id}" ${domain.id === card?.domainId ? "selected" : ""}>${escapeHtml(domain.name)}</option>`).join("")}</select></label><label class="form-field"><span>Nível *</span><select data-compendium-card-tier>${[1, 2, 3, 4].map((tier) => `<option value="${tier}" ${tier === (card?.tier ?? 1) ? "selected" : ""}>Nível ${tier}</option>`).join("")}</select></label><label class="form-field"><span>Tipo *</span><select data-compendium-card-type>${(["acao", "reacao", "passiva"] as const).map((type) => `<option value="${type}" ${type === (card?.cardType ?? "acao") ? "selected" : ""}>${type}</option>`).join("")}</select></label><label class="form-field"><span>Custo de uso</span><input data-compendium-card-cost value="${escapeHtml(card?.cost ?? "")}" placeholder="Ex.: 1 Esperanca" /></label><label class="form-field"><span>Custo de recall</span><input data-compendium-card-recall-cost type="number" min="0" step="1" value="${card?.recallCost ?? 0}" /><small>Stress para trazer esta carta do Vault fora de um descanso.</small></label></div><label class="form-field"><span>Imagem</span><input data-compendium-card-image type="file" accept="image/png,image/jpeg,image/webp" /><small>${card?.image ? "Uma imagem ja esta associada; envie outra para substitui-la." : "PNG, JPG ou WebP; ate 1,5 MB."}</small></label><label class="form-field"><span>Efeito *</span><textarea data-compendium-card-effect placeholder="Descreva a regra e o efeito completo da carta.">${escapeHtml(card?.effect ?? "")}</textarea></label></div><footer class="compendium-form-footer"><p class="form-error" data-compendium-card-error hidden></p><div class="modal-actions"><button class="sf-action sf-action--secondary secondary-action" type="button" data-modal-close aria-label="Cancelar" title="Cancelar">Cancelar</button><button class="sf-action sf-action--primary primary-action" type="button" data-action="save-compendium-card" aria-label="Gravar carta" title="Gravar carta">Salvar</button></div></footer></section></div>`;
}
...
export function renderDeleteCompendiumCardModal(dependencies: CardFeatureDependencies): string {
  ...
  return `<div class="modal-backdrop" data-modal-backdrop><section class="container-modal danger-modal" role="dialog" aria-modal="true" aria-labelledby="delete-card-title"><button class="modal-close" type="button" data-action="cancel-delete-compendium-card" aria-label="Cancelar exclusão">×</button><span class="resource-modal-label">Compendium</span><h2 id="delete-card-title">Excluir carta?</h2><p>A carta <strong>${escapeHtml(card.name)}</strong> ${isInDeck ? "está vinculada ao personagem atual." : "será removida deste dispositivo."}</p><div class="danger-summary"><strong>!</strong><span>${isInDeck ? "Remova a carta do deck antes de excluí-la." : "Esta ação não pode ser desfeita."}</span></div><div class="confirmation-actions"><button class="sf-action sf-action--secondary secondary-action" type="button" data-action="cancel-delete-compendium-card">Cancelar</button>${isInDeck ? "" : '<button class="sf-action sf-action--danger danger-action" type="button" data-action="confirm-delete-compendium-card">Excluir carta</button>'}</div></section></div>`;
}
...
function renderCompendiumCardResult(card: CardDefinition, dependencies: CardFeatureDependencies): string {
  ...
  return `<article class="compendium-card-result"><button class="compendium-card-result-open" type="button" data-card-modal-id="${card.id}" aria-label="Ver detalhes de ${dependencies.escapeHtml(card.name)}"><span class="compendium-card-result-image ${card.image ? "has-image" : ""}" ${card.image ? `style="background-image: url('${dependencies.escapeHtml(card.image)}')"` : ""} aria-hidden="true"></span><div class="compendium-card-result-copy"><div class="compendium-card-result-kicker"><span style="--domain-color: ${dependencies.escapeHtml(domain?.color ?? "#c39bff")}">${dependencies.escapeHtml(domain?.name ?? "Sem dominio")}</span><span>Nível ${card.tier}</span></div><h2>${dependencies.escapeHtml(card.name)}</h2><p>${dependencies.escapeHtml(card.summary)}</p></div><div class="compendium-card-result-meta"><span>${dependencies.escapeHtml(card.cardType)}</span><span>${dependencies.escapeHtml(card.cost ?? "Sem custo")}</span><span>⚡ Recall ${card.recallCost ?? 0}</span></div></button><div class="compendium-card-result-actions">${isLocal ? `<button class="sf-action sf-action--secondary sf-action--compact" type="button" data-action="edit-compendium-card" data-card-id="${card.id}">Editar</button><button class="sf-action sf-action--danger sf-action--compact" type="button" data-action="delete-compendium-card" data-card-id="${card.id}">Excluir</button>` : `<span class="readonly-label">Conteudo do pack</span><button class="sf-action sf-action--secondary sf-action--compact" type="button" data-action="edit-pack-card-marker" data-card-id="${card.id}">${card.gameMarkers?.length ? "Editar marcador" : "Complementar marcador"}</button>`}</div></article>`;
}


function readDefinitionImage(selector: string): Promise<string | undefined> {
  return readLocalImage(document.querySelector<HTMLInputElement>(selector)?.files?.[0]);
}
