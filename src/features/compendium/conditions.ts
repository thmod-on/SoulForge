...
function renderConditionDeleteModal(deps: ConditionFeatureDependencies): string {
  ...
  return entry ? `<div class="modal-backdrop" data-modal-backdrop><section class="container-modal danger-modal" role="dialog" aria-modal="true" aria-labelledby="delete-compendium-condition-title"><button class="modal-close" type="button" data-action="cancel-delete-compendium-condition" aria-label="Cancelar exclusão">×</button><span class="resource-modal-label">Compendium</span><h2 id="delete-compendium-condition-title">Excluir condição?</h2><p>“${deps.escapeHtml(entry.name)}” será removida deste dispositivo.</p><div class="danger-summary"><strong>!</strong><span>Esta ação não pode ser desfeita.</span></div><div class="confirmation-actions"><button class="sf-action sf-action--secondary secondary-action" type="button" data-action="cancel-delete-compendium-condition">Cancelar</button><button class="sf-action sf-action--danger danger-action" type="button" data-action="confirm-delete-compendium-condition">Excluir condição</button></div></section></div>` : "";
}
