...
export function renderTransformationFormModal(deps: TransformationFeatureDependencies): string {
...
function renderTransformationDeleteModal(deps: TransformationFeatureDependencies): string {
  ...
  return entry ? `<div class="modal-backdrop" data-modal-backdrop><section class="container-modal danger-modal" role="dialog" aria-modal="true" aria-labelledby="delete-compendium-transformation-title"><button class="modal-close" type="button" data-action="cancel-delete-compendium-transformation" aria-label="Cancelar exclusão">×</button><span class="resource-modal-label">Compendium</span><h2 id="delete-compendium-transformation-title">Excluir transformação?</h2><p>“${deps.escapeHtml(entry.name)}” será removida deste dispositivo.</p><div class="danger-summary"><strong>!</strong><span>Esta ação não pode ser desfeita.</span></div><div class="confirmation-actions"><button class="sf-action sf-action--secondary secondary-action" type="button" data-action="cancel-delete-compendium-transformation">Cancelar</button><button class="sf-action sf-action--danger danger-action" type="button" data-action="confirm-delete-compendium-transformation">Excluir transformação</button></div></section></div>` : "";
}
