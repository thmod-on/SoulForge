...
function renderRemoveConfirmation(deps: CharacterTransformationDependencies): string {
  ...
  return `<div class="modal-backdrop" data-modal-backdrop><section class="container-modal danger-modal" role="dialog" aria-modal="true" aria-labelledby="remove-character-transformation-title"><button class="modal-close" type="button" data-action="cancel-remove-character-transformation" aria-label="Cancelar remoção">×</button><span class="resource-modal-label">Alteração de identidade</span><h2 id="remove-character-transformation-title">Remover transformação?</h2><p><strong>${deps.escapeHtml(label)}</strong> deixará de aparecer e suas mecânicas serão desativadas.</p><div class="danger-summary"><strong>!</strong><span>Marcadores já registrados serão preservados, mas ficarão inativos.</span></div><div class="confirmation-actions"><button class="sf-action sf-action--secondary secondary-action" type="button" data-action="cancel-remove-character-transformation">Cancelar</button><button class="sf-action sf-action--danger danger-action" type="button" data-action="confirm-remove-character-transformation">Remover transformação</button></div></section></div>`;
}
