...
function renderItemTile(quantity: number, item: ItemDefinition, selected: boolean, equipped: boolean, compartmentId: string, dependencies: InventoryRenderDependencies): string {
  const { escapeHtml, itemFilterLabels } = dependencies;
  return `<button class="item-tile ${selected ? "is-active" : ""}" data-item-id="${item.id}" data-item-compartment-id="${compartmentId}" draggable="false"><span class="item-media">${renderItemVisual(item, "tile", escapeHtml)}<span class="item-quantity">x${quantity}</span></span><strong>${escapeHtml(item.name)}</strong><small>${item.tier ? `Tier ${item.tier}` : itemFilterLabels[item.category]} - ${itemFilterLabels[item.category]}</small>${equipped ? "<em>Equipado</em>" : ""}</button>`;
}
...
export function renderItemModal(dependencies: InventoryRenderDependencies): string {
  ...
  const selectedEntry = getItemEntries(character).find(({ item }) => item.id === state.selectedItemId);
