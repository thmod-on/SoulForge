...
export function renderAddItemToContainerModal(dependencies: InventoryRenderDependencies): string {
  ...
  if (previewItem) {
    return `<div class="modal-backdrop" data-modal-backdrop><section class="container-modal add-item-modal add-item-detail-modal" role="dialog" aria-modal="true" aria-labelledby="add-item-preview-title"><div class="container-modal-heading"><h2 id="add-item-preview-title">${escapeHtml(previewItem.name)}</h2><button class="modal-close modal-close-inline" data-modal-close aria-label="Fechar adicionar item">x</button></div><p>Confira os detalhes antes de adicionar em <strong>${escapeHtml(compartment.name)}</strong>.</p><article class="add-item-preview"><div class="add-item-preview-art">${renderItemVisual(previewItem, "detail", escapeHtml)}</div><div class="add-item-preview-body"><span class="resource-modal-label">${itemFilterLabels[previewItem.category]}</span><span>${previewItem.tier ? `Tier ${previewItem.tier}` : "Sem Tier"}</span><p>${escapeHtml(previewItem.summary)}</p><dl class="detail-list item-modal-details"><div><dt>Valor</dt><dd>${previewItem.value ?? "-"}</dd></div><div><dt>Peso</dt><dd>${previewItem.weight}</dd></div></dl><div class="trait-list">${(previewItem.traits ?? []).map((trait) => `<span>${escapeHtml(trait)}</span>`).join("")}</div>${renderCombatModifiers(previewItem)}<div class="add-item-preview-actions"><button class="secondary-action" type="button" data-action="close-add-item-preview">Voltar à lista</button><button class="primary-action" type="button" data-action="select-add-item-from-preview" data-item-id="${previewItem.id}">Selecionar item</button></div></div></article></section></div>`;
  }
  return `<div class="modal-backdrop" data-modal-backdrop><section class="container-modal add-item-modal" role="dialog" aria-modal="true" aria-labelledby="add-item-title"><div class="container-modal-heading"><h2 id="add-item-title">Adicionar item</h2><button class="modal-close modal-close-inline" data-modal-close aria-label="Fechar adicionar item">x</button></div><p>Escolha um item do Compendium para adicionar em <strong>${escapeHtml(compartment.name)}</strong>.</p><label class="sf-search-field add-item-search"><span aria-hidden="true">⌕</span><input type="search" placeholder="Buscar item..." aria-label="Buscar item" data-add-item-catalog-search value="${escapeHtml(state.addItemCatalogSearch)}" /></label><div class="add-item-filter-stack"><div class="filter-row add-item-filter-row">${(Object.keys(itemFilterLabels) as InventoryFilter[]).map((category) => `<button class="chip sf-filter-option ${state.addItemCatalogFilter === category ? "is-active" : ""}" type="button" data-add-item-filter="${category}">${itemFilterLabels[category]}</button>`).join("")}</div><div class="filter-row add-item-filter-row add-item-tier-filter"><span>Nível</span>${tierOptions.map((option) => `<button class="chip sf-filter-option ${state.addItemCatalogTierFilter === option.value ? "is-active" : ""}" type="button" data-add-item-tier-filter="${option.value}">${option.label}</button>`).join("")}</div></div><div class="add-item-catalog">${availableItems.length ? availableItems.map((item) => `<button class="item-tile add-item-choice ${state.addingDefinitionItemId === item.id ? "is-active" : ""}" type="button" data-add-item-preview-definition-id="${item.id}" aria-label="Ver detalhes de ${escapeHtml(item.name)}"><span class="item-media">${renderItemVisual(item, "tile", escapeHtml)}${state.addingDefinitionItemId === item.id ? "<span class=\"item-selection-state\">Selecionado</span>" : ""}</span><strong>${escapeHtml(item.name)}</strong><small>${item.tier ? `Tier ${item.tier} - ` : ""}${itemFilterLabels[item.category]} · Peso ${item.weight}</small></button>`).join("") : renderEmptyInline("Nenhum item compatível com estes filtros.")}</div>${selectedItem ? `<div class="add-item-confirm"><label><span>Quantidade</span><input type="number" min="1" step="1" value="1" data-add-item-quantity /></label><span>${fitsSelected ? "O item cabe neste container." : "Não há capacidade suficiente neste container."}</span><button class="primary-action" type="button" data-action="confirm-add-item-to-container">Adicionar</button></div>${state.addItemError ? `<p class="form-error">${escapeHtml(state.addItemError)}</p>` : ""}` : ""}</section></div>`;
}
